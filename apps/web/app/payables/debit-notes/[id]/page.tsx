...
import type { DebitNote } from "../../../../lib/api";
...
export default function CreditNoteDetailPage() {
  ...
  const cn = useQuery({
    ...
    queryKey: ["debitNote", id],
    queryFn: () => api.getDebitNote(id),
  });

  const auditQ = useQuery({

    queryKey: ["audit-debitNote", id],
    queryFn: () => api.auditLogFor("DebitNote", id),
  });
  ...
  const remove = useMutation({
    ...
    mutationFn: () => api.deleteDebitNote(id),
    onSuccess: () => {

      qc.invalidateQueries({ queryKey: ["debitNotes"] });
      window.location.href = "/payables/debit-notes";
    },
  ...
  if (cn.isLoading) return <p className="p-8 text-slate-500">Loading debit note…</p>;
  if (cn.error || !cn.data)
  ...
  return (
    ...
    <div>
      ...
      <PageHeader
        ...
        title={"Debit Note " + c.number}
        description={"Supplier: " + (c.supplier?.name ?? c.supplierId)}
        descriptionHref={"/payables/suppliers/" + c.supplierId}
        breadcrumbs={[

          { label: "Payables", href: "/payables" },
          { label: "Debit Notes", href: "/payables/debit-notes" },
          { label: c.number ?? "" },
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() => {
                ...
                if (confirm("Delete debit note " + c.number + "? This reverses the GL posting.")) remove.mutate();
              }}
            ...
            <Link href="/payables/debit-notes">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <Stat label="Related bill" value={c.invoice ? c.invoice.number : "—"} bold={!!c.invoice} />
      ...
      <div className="mt-6 flex items-center gap-2 rounded-lg border bg-white px-4 py-3 text-xs text-slate-500">
        ...
        <span>Debit notes can be submitted to MyInvois as a debit/refund document.</span>
        {c.invoice && (

          <Link href={"/payables/" + c.invoice.id} className="font-medium text-brand-700 hover:underline">
            Open related bill →
          </Link>
