...
export class ApService {
  ...
  constructor(private readonly prisma: PrismaService, private readonly seq: DocumentSequenceService, private readonly posting: PostingService) {}
  ...
  async listInvoices(bookId: string, page = 1, pageSize = 50, supplierId?: string, status?: string) {
    ...
    const [data, total] = await Promise.all([
      ...
      this.prisma.supplierInvoice.count({ where: where as never }),
  ...
  async createInvoice(bookId: string, dto: CreateSupplierInvoiceDto): Promise<Record<string, unknown>> {
    ...
    const number = await this.seq.next(bookId, "SINV");
    ...
    try {
      await this.posting.postSupplierInvoice(created.id);
    } catch (err) {
      this.logger.warn(`GL post skipped for bill ${created.number}: ${(err as Error).message}`);
    }
